/* Copyright (c) 2015 Avalos Corp. All rights reserved. */
...
Ext.define("SYNO.SDS.UIFeatures.IconSizeManager", {
    statics: {
        ...
        getIconPath: function(f, g) {
            var j, b, e = "/webman/",
                c = "/synohdpack/images/dsm/";
            var h = this.getRetinaAndSynohdpackStatus();
            var d = function(l, n, m, k) {
                return l.replace(n, (n === "48") ? "128" : n * 2)
            };
            var a = function(l, n, m, k) {
                return l.replace(n, (n === "48") ? "128" : n * 2)
            };
            if (0 === f.indexOf("3rdparty/")) {
                var i = String.format("/webman/synohdpack.cgi?method={0}&res={1}&retina={2}&path={3}", "getHDIcon", this.getRes(g), h, "webman/") + f;
                return i
            }
            if (h) {
                if (0 === f.indexOf(e)) {
                    b = c + f.substr(e.length)
                } else {
                    b = c + f
                }
            } else {
            ...
            switch (g) {
                case "Taskbar":
                    j = String.format(b, h ? this.Taskbar * 2 : this.Taskbar);
                    break;
                case "Desktop":
                    ...
                    if (-1 != b.indexOf("files_ext_")) {
                        j = h ? b.replace(/.*\/files_ext_(\d+)\/.*/, d) : b
                    } else {
                        if (-1 != b.indexOf("shortcut_icons")) {
                            j = h ? b.replace(/.*\/.*_(\d+)\.png$/, a) : b
                        } else {
                            j = String.format(b, h ? 256 : this.Desktop)
                        }
                    }
                    break;
                case "ClassicalDesktop":
                    if (-1 != b.indexOf("files_ext_")) {
                        ...
                        if (-1 != b.indexOf("shortcut_icons")) {
                            ...
                            j = String.format(b, h ? 256 : this.ClassicalDesktop)
                        }
                    }
                    break;
                case "AppView":
                    j = String.format(b, h ? 256 : this.AppView);
                    break;
                case "AppViewClassic":
                    j = String.format(b, h ? 256 : this.AppViewClassic);
                    break;
                case "Header":
                    j = String.format(b, h ? this.Header * 2 : this.Header);
                    break;
                case "HeaderV4":
                    j = String.format(b, h ? this.HeaderV4 * 2 : this.HeaderV4);
                    break;
                case "StandaloneHeader":
                    j = String.format(b, h ? this.StandaloneHeader * 2 : this.StandaloneHeader);
                    break;
                case "FavHeader":
                    j = String.format(b, h ? this.FavHeader * 2 : this.FavHeader);
                    break;
                case "FileType":
                    j = (h) ? b.replace(/.*\/files_ext_(\d+)\/.*/, d) : b;
                    break;
                case "PortalIcon":
                    j = String.format(b, h ? this.PortalIcon * 2 : this.PortalIcon);
                    break;
                case "TreeIcon":
                    j = String.format(b, h ? this.TreeIcon * 3 : this.TreeIcon);
                    break;
                default:
                    j = b;
                    break
...
SYNO.SDS._UserSettings = Ext.extend(Ext.Component, {
    ...
    save: function() {
        ...
        this.ajaxTask = this.addWebAPITask({
            method: "apply",
            single: true,
            version: 1,
            api: "SYNO.Core.UserSettings",
    ...
    onRender: function() {
        ...
        Ext.each(a, function(b) {
            ...
            if ("sds-taskbar-user-button" === c) {
                ...
                if ("sds-taskbar-search-button" === c) {
                    ...
                    if ("sds-taskbar-preview-button" === c) {
                        if (SYNO.SDS.UIFeatures.test("exposeWindow")) {
                            ...
                            this.previewButton.mon(SYNO.SDS.StatusNotifier, "taskbuttonchanged", function() {
                                ...
                                this.previewButton.setDisabled((d.length === 0))
                            }, this);
...
Ext.define("SYNO.SDS.SRM1Logo", {
    extend: "SYNO.SDS.DSM5Logo",
    logoArray: ["synology", "SRM", "1dot"],
    version: 0,
    isBeta: false
});
var isNSM = (_D("syno_os") === "nsm");
SYNO.SDS.OS_Logo = Ext.getClassByName(isNSM ? "SYNO.SDS.SRM1Logo" : "SYNO.SDS.DSM5Logo");
...
Ext.define("SYNO.SDS.FullScreenToolbar", {
    ...
    constructor: function(a) {
        if (!a.appWin || !(a.appWin instanceof SYNO.SDS.AppWindow)) {
            SYNO.Debug("Need to add appWin in config!");
            return
        }
...
Ext.define("SYNO.SDS.MessageBoxV5", {
    ...
    selectBtnByTab: function(c) {
        var b, a;
        c.preventDefault();
        for (b = this.buttons.length - 1; b >= 0; b--) {
            if (this.buttons[b] === this.focusEl) {
                break
            }
            if (!this.buttons[b].hidden) {
                a = this.buttons[b]
            }
        }
        if (!a) {
    ...
    selectBtn: function(a) {
        var b = this.focusEl[a + "Sibling"]();
        while (b && b.hidden) {
            b = b[a + "Sibling"]()
        }
        if (!b || b.hidden) {
                ...
                listeners: {
                    afterrender: function() {
                        ...
                        this.nav = new Ext.KeyNav(this.btnEl, {
                            left: this.msgWin.selectBtn.createDelegate(this.msgWin, ["previous"]),
                            right: this.msgWin.selectBtn.createDelegate(this.msgWin, ["next"]),
                            tab: this.msgWin.selectBtnByTab.createDelegate(this.msgWin)
                        });
                        this.mon(this.btnEl, {
...
SYNO.SDS.WindowLaunch = function(k, e, j, g) {
    var f = SYNO.SDS.Config.FnMap[k],
        n = "";
    if (!f) {
        return false
    }
    if (j === false) {
        ...
        c.addClass("sds-window-v5");
        c.addClass("sds-standalone-desktop");
        c.mask(_WFT("common", "loading"), "x-mask-loading x-standalone-loading")
    }
    if ("standalone" === f.config.type || true === f.config.allowStandalone || "url" === f.config.type || "legacy" === f.config.type) {
        if (!f.config.url && !SYNO.SDS.UrlTag[f.config.urlTag]) {
            return false
        }
        if (Ext.isObject(e)) {
            n = {
                launchParam: Ext.urlEncode(e)
            };
            if (Ext.isDefined(e.ieMode)) {
                n.ieMode = e.ieMode
            }
        }
        var b = (g && g.url) ? g.url : f.config.url || SYNO.SDS.UrlTag[f.config.urlTag] || "";
        if (SYNO.SDS.QuickConnect.Utils.isInTunnel() && !SYNO.SDS.IsDSMPortalWhiteList(k) && "url" === f.config.type) {
        ...
        if ((f.config.port || f.config.protocol) && ("http" !== b.substr(0, 4).toLowerCase())) {
            var m = f.config.protocol || window.location.protocol;
            var d = f.config.port || window.location.port || "";
            if (d) {
                d = ":" + d
            }
            ...
            b = m + "://" + window.location.hostname + d + b
        }
        var l = b.split("?"),
            h, i;
        if (l.length > 2) {
            l[1] = l.splice(1, l.length - 1).join("?")
        }
        h = l[1] ? Ext.urlDecode(l[1]) : {};
        i = Ext.apply(h, n);
        if (false === j) {
            window.location = Ext.urlAppend(l[0], Ext.urlEncode(i), false)
        } else {
            window.open(Ext.urlAppend(l[0], Ext.urlEncode(i), false), f.config.urlTarget ? f.config.urlTarget + SYNO.SDS.LaunchTime : "_blank", f.config.windowParam || "")
        }
        return true
    }
    return false
...
SYNO.SDS.AppLaunch = function(className, param, newInstance, callback, scope) {
    ...
    if (_S("standalone")) {
        ...
        el.addClass("sds-standalone-desktop");
        ...
        this.addAjaxTask({
            ...
            success: function(d, f) {
                ...
                if (d.success === false && d.errinfo) {
                    this.getMsgBox().alert(_D("product"), _JSLIBSTR(d.errinfo.sec, d.errinfo.key), function(h) {
                        SYNO.SDS.Desktop.show()
                    }, this);
                    return
                }
            ...
            listeners: {
                afterrender: {
                    fn: function() {
                        ...
                        this.osLogo = new SYNO.SDS.OS_Logo(d);
...
Ext.override(SYNO.ux.DateField, {
    ...
    onTriggerClick: function() {
        ...
        this.callParent(arguments);
        if (!this.disabled && !this.menu.el.hasClass("syno-ux-datefield-menu")) {
            this.menu.addClass("syno-ux-datefield-menu")
        }
...
Ext.define("SYNO.SDS.WeatherInfo", {
    ...
    constructor: function(a) {
        ...
        this.welcomeMsgMode = a.welcomeMsgMode || false;
        var b = {
            id: "sds-weather-info",
            cls: (this.welcomeMsgMode) ? "weather-welcome-mode" : "",
            items: [{
                xtype: "displayfield",
                cls: "weather-temp",
                hideMode: "visibility"
    ...
    getTimeStr: function() {
        ...
        var b;
        if (this.format === "24h") {
            b = String.format('{0}<span class="minute">:{1}</span>', a.format("H"), a.format("i"))
        } else {
            b = String.format('{0}<span class="minute">:{1}</span>', a.format("g"), a.format("i"));
    ...
    getDateStr: function() {
        ...
        var g = i.getMonth() + 1,
            ...
            e = i.getDate();
        var f = i.getFullYear();
        var c = _T("login", "mon_" + g);
        var a = _T("login", "weekday_" + h);
        var b;
        if (this.welcomeMsgMode) {
            b = String.format('<div class="date-tag">{0}</div><div class="week-tag">{1}</div>', String.format(_T("login", "date_format_without_week"), c, e.toPaddedString(2)), a)
        } else {
            b = String.format(_T("login", "date_format_with_year"), c, e, a, f)
        }
        return b
    },
    defineFileds: function() {
        ...
        this.timeField = this.items.items[2].items.items[0];
        this.dateField = this.items.items[2].items.items[1];
...
SYNO.SDS.Utils.AddTip = function(c, j) {
    ...
    var f = SYNO.SDS.UIFeatures.test("isRetina") ? SYNO.SDS.ThemeProvider.getPath("/resources/images/components/icon_information_mini.png") : SYNO.SDS.ThemeProvider.getPath("/synoSDSjslib/images/components/icon_information_mini.png");
...
SYNO.SDS.Utils.CheckStrongPassword = Ext.extend(Object, {
    ...
    isPasswordValid: function(p, n, b) {
        ...
        if (q === "admin" && p.length < 6) {
            return _T("passwd", "min_length_enable") + "6"
        }
