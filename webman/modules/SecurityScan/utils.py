...
class DictResult(object):
    ...
    def _statusRetCheck(self, val):
        ...
        if val not in statusRet:
            SYSLOG(0, "The value of status must in %s" % " ".join(statusRet))
            return False
        return True
    ...
    def _methodRetCheck(self, val):
        ...
        '''
        if dict != type(val):
            SYSLOG(0, "The value of methodSet() must be dict")
            return False
        if not val.has_key(METHOD_ACTION):
            SYSLOG(0, "The value of methodSet() must have %s" % METHOD_ACTION)
            return False
        return True
    ...
    def _actionRetCheck(self, val):
        ...
        if dict != type(val):
            SYSLOG(0, "The value of actionSet() must be dict")
            return False

        if not val.has_key(ACTION_STR_KEY):
            SYSLOG(0, "The value of actionSet() must have %s" % ACTION_STR_KEY)
            return False
...
def checkEnabled(strServiceName):
    synoSevcmd = '/usr/syno/sbin/synoservicecfg --is-enabled %s' % (strServiceName)
