    ...
    try:
        ...
        if (1 == len(versionAndNumber)): #'1.0' or '1' are possible values
            ...
            if (2 < len(majorAndMinorVersion)):
                SYSLOG(3, "must have main and minor, _non_comp_version foramt is wrong:" + notRangeStyle)
                return -1
            else:
...
def isVersionMatched(versionExp):
    ...
    if (2 < len(rangeFromTo)): # do not allow xxx~xxx~xxx or more ~
        SYSLOG(3 ,"too many ~ ,_non_comp_version foramt is wrong:" + str(versionExp))
        return False
...
def initGVdict():
    ...
    with open(dsmVersionFile) as fp:
        for line in fp:
            ...
            if (2 != len(dlist)):
                SYSLOG(3, "parsing" + dsmVersionFile + "error")
                return False
...
def verCheck(strVersionList):
    ...
    try:
        ...
        SYSLOG(0, "Failed to runnable() Error:%s" % e)
        trace = traceback.format_exc()
        tracelog(trace)
...
def enumItems(group = '', runableCheck = True):
    ...
    try:
        ...
        with open(DB_LIST_PATH) as fp:
            ...
            DBHash = DBHash[CHECKSUM_FILE_RULE_KEY]
        ...
        SYSLOG(0, "Faile to enumItems %s" % e)
        return []
...
def scanItemsEnum(argRules):
    ...
    if ITEMS_ALL == rules:
        ...
        if GROUP_CUSTOM == sysSettings[CONFIG_DEF_GROUP_KEY]:
            if os.path.isfile(CUSTOM_LIST_FILE):
                with open(CUSTOM_LIST_FILE) as fp:
                    rules = fp.read()
            else:
...
def defaultItemGet(itemID):
    ...
    try:
        ...
        SYSLOG(0, "Fail to defaultItemGet() Error:%s" % e)
        return {}
...
def crontabUpdate(blSchedule, weekday = '', hour = '', minute = ''):
    import ctypes
    api = ctypes.CDLL(SYNOSDK_LIBRARY_PATH)
    api.SYNOScheduleRemove.argtypes = [ctypes.c_char_p]
    api.SYNOScheduleSet.argtypes = [ctypes.POINTER(SYNOSCHEDULE), ctypes.c_int]
    api.SYNOScheduleRemove(SYSTEM_SYNOSECURITY_SCAN_CMD)
    return True
    ...
    if isMainPyAlive() and not isUpdating():
        ...
        try:
            ...
            try:
                ...
                SYSLOG(0, "Failed to itemDataGet() Error %s" %e)
                return False
...
def updateCheck():
    ...
    try:
        jsonData = execute('curl --connect-timeout %d %s' % (UPDATE_SOCKET_TIMEOUT, VERSION_URL), blSplit=False)
        ...
        SYSLOG(0, "Fail to get version %s" % e)
        return 0, 0, False
...
def downloadFile(src, dest):
    ...
    try:
        ...
        SYSLOG(0, "Failed to downloadFile() Error: %s" % e)
        return 1
...
def dirMove(src, dst):
    ...
    try:
        ...
        SYSLOG(0, "Failed to dirMove(%s, %s) Error: %s" % (src, dst, e))
...
def updateSecurityDB(latestVer):
    ...
    try:
        ...
        if 0 != downRet:
            SYSLOG(3, "Failed to download database from %s" % downloadURL)
            return ret
        ...
        if 0 != cmdRet:
            SYSLOG(3, "The signature of %s is wrong" % DB_DOWNLOAD_SAVE_PATH)
            return ret

        dbTar = tarfile.open(DB_PATH_TAR_TMP)
        ## untar db to DB_PATH_TMP
        if os.path.exists(DB_PATH_TMP):
            shutil.rmtree(DB_PATH_TMP)
        dbTar.extractall(DB_PATH_TMP)
        dbTar.close()
        os.unlink(DB_PATH_TAR_TMP)
        ...
        try:
            ...
            SYSLOG(3, "Security Adviser DB have updated to version %s" % latestVer)
            ret = True
        except Exception as e:
            ## Restore when there are any error durring changes new db and strings
            SYSLOG(0, "Failed to update new database and strings %s" % e)
        ...
        SYSLOG(3, "Failed to updateSecurityDB() Error: %s" % e)
        return False
...
def utcTimeGet():
    ...
    try:
        ...
        SYSLOG(0, "Failed to utcTimeGet() Error: %s" % e)
        return ""
...
def sysStatusCalculate(currentData, rules):
    ...
    if not currentData:
        ...
        try:
            ...
            SYSLOG(0, "Failed to _sysStatusItemGet() Error: %s" % e)
            trace = traceback.format_exc()
            tracelog(trace)
...
def resultSet(module, itemID, blFail=False):
    ...
    try:
        ...
        result = {}
        errorSet(result)
        SYSLOG(0, "Failed to check rules %s Error: %s" % (itemID, e))
    finally:
...
def moduleImport(module):
    ...
    try:
        ...
        SYSLOG(0, "Failed to import module %s" % e)
        return False
...
def checkTime():
    ...
    try:
        ...
        if time.time() < mTime:
            SYSLOG(0, "System time %f < gpg list %s time %f" % (time.time(), DB_LIST_GPG_PATH, mTime))
            return False
        return True
    except Exception as e:
        SYSLOG(0, "Failed to checkTime: %s" % e)
        return False
...
def checksumGen():
    ...
    if 0 != cmdRet:
        SYSLOG(0, "The signature of %s is wrong" % GPG_DECRYPTION_CHECKSUM)
        sysStatusDangerSet()
        return False
    ...
    with open (DB_LIST_PATH) as fp:
        ...
        if md5Table[CHECKSUM_FILE_SO_KEY] != checksumCalc(DB_SO_PATH):
            SYSLOG(0, "The checksum of %s is incorrect" % DB_SO_PATH)
            return False
    return True
        ...
        for i in range(maxRetry):
            try:
                ...
                if 111 == e.errno and blExcept:
                    ...
                    continue
        if not blConnected:
            SYSLOG(0, "Failed to connect to %s" % IPC)
            raise
...
class SYNOSCHEDULE(ctypes.Structure):
    #==========================Ctypes for SDK============================#
    '''
    Clone the following structure in libsynosdk/includes/synosdk/crontab.h

    typedef struct _tag_SYNO_SCHEDULE{
        int iWDays;    /*day of week, 0-7, 0 is Sun, 7 is everyday */
        int iHourStart;   /*0-23*/
        int iHourInterval; /*0-23*/
        int iHourLast;   /*0-23*/
        int iMinStart;        /*0-59*/
        int iIteration;        /*1-24*/
        char szCommand[1024];  /* crontab command, cannot be NULL*/
    ...
    def __init__(self):
        self.iWDays = 0
        self.iHourStar = 0
        self.iHourInterval = 0
        self.iHourLast = 0
        self.iMinStart = 0
        self.iIteration = 0

    _fields_ = [('iWDays', ctypes.c_int),
            ('iHourStart', ctypes.c_int),
            ('iHourInterval', ctypes.c_int),
            ('iHourLast', ctypes.c_int),
            ('iMinStart', ctypes.c_int),
            ('iIteration', ctypes.c_int),
            ('szCommand', ctypes.c_char * COMMANDSIZE)]
