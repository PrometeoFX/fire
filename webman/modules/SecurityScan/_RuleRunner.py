...
try:
    ...
    SYSLOG(0, "Failed to open %s Error %s" % (DB_LIST_PATH, e))
    exit(1)
...
def sigHandler(sig, frame):
    try:
        ...
        SYSLOG(0, "Fail to exit() Error: %s" % e)
    ...
    if blNotify:
        ...
        if SYSTEM_STATUS_DANGER == sysStatus:
            ...
            if bldsmUpdateFail and blpkgUpdateFail:
                ...
                SYSLOG(0, "Unexpect notify in %s" % SYSTEM_STATUS_OUT_OF_DATE)
...
def ruleCheck(itemID, module):
    ...
    if not module:
        SYSLOG(0, "Failed to import %s" % itemID)
        return False
    ...
    if gChecksumTable[itemID] != itemChecksumCalc(itemID):
        ...
        SYSLOG(0, "The checksum of file %s != original checksum %s" % (itemID, gChecksumTable[itemID]))
        return False
    ...
    try:
        ...
        if blFixme:
            if CMD_FIXME_ACTION not in dir(moduleObj):
                SYSLOG(0, '%s has not implement fixmeAction but called' % itemID);
                return

            result = moduleObj.fixmeAction()
            if  True != result:
                SYSLOG(0, 'Failed to fix item %s' % itemID)
                return

        result = resultSet(moduleObj, itemID)
        gMsgQueue.put(result)
    except Exception as e:
        SYSLOG(0, "Failed to exec rules %s Error: %s" % (itemID, e))
...
def updateCheckItems(items):
    ...
    ## update
    for itemID in items:
        if itemID in gItemsDict:
            gItemsDict[itemID][RESULT_STATUS] = STATUS_RUNNING
            itemData = gItemsDict[itemID]
        else:
            gItemsDict[itemID] = {}
...
def wakeup(sk):
    ...
    try:
        ...
        SYSLOG(1, "Capture exception Error: %s" % e)
        trace = traceback.format_exc()
        tracelog(trace)
    except KeyboardInterrupt as e:
        SYSLOG(1, "Exit by Ctrl-C")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if 0 == pid:    ## Child PID
            ...
            SYSLOG(0, "Failed to wakeup()")
            exit(1)
    except Exception as e:
        SYSLOG(0, "Failed to call _RuleRunner.py" % e)
