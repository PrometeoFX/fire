...
def start(rules, blNotify):
    ...
    try:
        ...
        resp[RETURN_SUCCESS_KEY] = False
        SYSLOG(0, 'Failed to scan Error: %s' % e)
...
def sysStatus():
    ...
    try:
        ...
        SYSLOG(0, "Failed to sysStatus() Error: %s" % e);
        resp[RETURN_SUCCESS_KEY] = False
...
def confSave(args):
    ...
    try:
        ...
        if GROUP_CUSTOM == argGroup and not os.path.exists(CUSTOM_LIST_FILE):
            ...
            # Remove old version schedule in crontab and config
            crontabUpdate(False)
            if sysSettings[CONFIG_SCHEDULE].has_key(CONFIG_ENABLE_SCHEDULE):
                del sysSettings[CONFIG_SCHEDULE][CONFIG_ENABLE_SCHEDULE]
            if sysSettings[CONFIG_SCHEDULE].has_key(CONFIG_SCHEDULE_WEEKDAY):
                del sysSettings[CONFIG_SCHEDULE][CONFIG_SCHEDULE_WEEKDAY]
            if sysSettings[CONFIG_SCHEDULE].has_key(CONFIG_SCHEDULE_HOUR):
                del sysSettings[CONFIG_SCHEDULE][CONFIG_SCHEDULE_HOUR]
            if sysSettings[CONFIG_SCHEDULE].has_key(CONFIG_SCHEDULE_MINUTE):
                del sysSettings[CONFIG_SCHEDULE][CONFIG_SCHEDULE_MINUTE]
        ...
        SYSLOG(0, "Fail to save config default group:%s updateBeforeScan:%s Error: %s" % (argGroup, updateBeforeScan, e))
        resp[RETURN_SUCCESS_KEY] = False
...
def customListSave(rules):
    ...
    if not rules:
        ...
        try:
            ...
            SYSLOG(0, "Fail to save custom list Error: " % err)
            resp[RETURN_SUCCESS_KEY] = False
    ...
    try:
        ...
        if 0 == latestVer and 0 == curVer:
            ...
            if resp[RETURN_UPDATING] or not needToUpdate:
                ...
                if (True == blUpdate):
                    ...
                    try:
                        if not updateSecurityDB(latestVer):
                            resp[RETURN_SUCCESS_KEY] = False
                            setErrInfo(resp, 'securityscan', 'securityscan_error_update')
                            SYSLOG(3, 'Failed to updateSecurityDB() but still running')
                    except Exception as e:
                        SYSLOG(0, "Failed to updateSecurityDB %s" % e)
                    finally:
                ...
                SYSLOG(3, "Failed to updateCheck()")

        sys.stdout.write(json.dumps(resp))
    except Exception as e:
        SYSLOG(0, "Failed to update Error: %s " % e)
        trace = traceback.format_exc()
        tracelog(trace)
...
def dispatcher(args):
    ...
    if "StartScan" == args.action:
        ...
        SYSLOG(0, "Bad request %s" % args.action)
...
if __name__ == "__main__":
    ...
    try:
        ...
        SYSLOG(0, "Failed to exec dispatcher() Error: %s" % e)
